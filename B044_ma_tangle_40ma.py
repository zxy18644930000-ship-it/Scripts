#!/usr/bin/env python3
"""
B044: 40MA 趋势确认 — 在B043双时间框架基础上增加40MA确认层

改进思路:
  B043用20MA LB10/T7判定trending, 但价格可能只是围绕40MA大幅震荡而非真趋势.
  新增40MA确认: 当20MA判定trending时, 检查40MA纠缠度:
    - 20MA=trending + 40MA=trending → 真趋势 (confirmed_trending)
    - 20MA=trending + 40MA=entangled → 降级回纠缠 (overridden_entangled)
    - 20MA=entangled → 纠缠 (不变)

  对1m和5m两个时间框架分别做40MA确认.

用法:
  python3 B044_ma_tangle_40ma.py --product ag
  python3 B044_ma_tangle_40ma.py --product ag,p,cu --workers 4
  python3 B044_ma_tangle_40ma.py --all --workers 12
"""
...
# ============ 配置 ============
MA_PERIOD_1M = 20
MA_PERIOD_1M_CONFIRM = 40   # 40MA确认层 (1分钟)
MA_PERIOD_5M = 20
MA_PERIOD_5M_CONFIRM = 40   # 40MA确认层 (5分钟)
LOOKBACK = 10           # 回看10根
...
RESULT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ma_tangle_40ma_results')
...
def compute_5m_states(df_1m, lookback, threshold, ma_period=None):
    """从1分钟bar重采样出5分钟bar, 计算MA状态

    跨session连续计算MA (和实际看盘一致), 不按session重启.
    非交易时段的NaN自然被dropna()移除, MA只看有效bar.
    """
    if ma_period is None:
        ma_period = MA_PERIOD_5M
    df = df_1m.copy()
...
def backtest_single_contract(futures_df, options_data, product, contract_yymm,
    ...
    min_warmup = max(MA_PERIOD_5M_CONFIRM, MA_PERIOD_5M) * 5 + lookback + max(hold_periods)
    if len(fdf) < min_warmup:
        return []

    # 1分钟 20MA 状态
    fdf = compute_ma_states(fdf, MA_PERIOD_1M, lookback, threshold)
    fdf['ma_state_20'] = fdf['ma_state'].copy()

    # 1分钟 40MA 确认层
    fdf_40 = compute_ma_states(fdf.copy(), MA_PERIOD_1M_CONFIRM, lookback, threshold)
    fdf['ma_state_40'] = fdf_40['ma_state']

    # 40MA降级: 20MA说trending但40MA说entangled → 降回entangled
    mask_1m_override = (fdf['ma_state'].str.startswith('trending')) & (fdf['ma_state_40'] == 'entangled')
    fdf.loc[mask_1m_override, 'ma_state'] = 'entangled'
    fdf['overridden_1m'] = mask_1m_override

    # 5分钟 20MA 状态
    state_5m = compute_5m_states(fdf, lookback, threshold, ma_period=MA_PERIOD_5M)
    fdf['state_5m'] = state_5m
    fdf['state_5m_20'] = state_5m.copy()

    # 5分钟 40MA 确认层
    state_5m_40 = compute_5m_states(fdf, lookback, threshold, ma_period=MA_PERIOD_5M_CONFIRM)
    fdf['state_5m_40'] = state_5m_40

    # 40MA降级: 5m 20MA说trending但5m 40MA说entangled → 降回entangled
    mask_5m_override = (pd.Series(fdf['state_5m'].values).str.startswith('trending').values) & \
                       (pd.Series(fdf['state_5m_40'].values) == 'entangled').values
    fdf.loc[mask_5m_override, 'state_5m'] = 'entangled'
    fdf['overridden_5m'] = mask_5m_override
    ...
    for idx in range(len(fdf)):
        ...
        trade = {
            ...
            'entry_sum': entry_sum,
            'state_1m': state_1m,
            'state_1m_raw': row['ma_state_20'],
            'state_5m': row['state_5m'],
            'state_5m_raw': row['state_5m_20'],
            'overridden_1m': bool(row['overridden_1m']),
            'overridden_5m': bool(row['overridden_5m']),
...
def process_product(product):
    ...
    prod_upper = product.upper()
    print(f'\n{"="*60}')
    print(f'B044 40MA Confirm: {prod_upper} ({exchange})')
    print(f'{"="*60}')
    ...
    result['params'] = {
        'ma_1m': MA_PERIOD_1M, 'ma_1m_confirm': MA_PERIOD_1M_CONFIRM,
        'ma_5m': MA_PERIOD_5M, 'ma_5m_confirm': MA_PERIOD_5M_CONFIRM,
        'lookback': LOOKBACK, 'threshold': THRESHOLD,
        'hold_periods': HOLD_PERIODS, 'entry_interval': ENTRY_INTERVAL,
    }

    n_overridden_1m = int(df_trades['overridden_1m'].sum()) if 'overridden_1m' in df_trades.columns else 0
    n_overridden_5m = int(df_trades['overridden_5m'].sum()) if 'overridden_5m' in df_trades.columns else 0
    _mask = (df_trades['overridden_1m'] | df_trades['overridden_5m']) if 'overridden_1m' in df_trades.columns else pd.Series(False, index=df_trades.index)
    n_any_override = int(_mask.sum())
    result['override_stats'] = {
        'overridden_1m': n_overridden_1m,
        'overridden_5m': n_overridden_5m,
        'any_override': n_any_override,
        'pct_of_total': round(n_any_override / len(df_trades) * 100, 1) if len(df_trades) > 0 else 0,
    }
    print(f'  40MA overrides: 1m={n_overridden_1m}, 5m={n_overridden_5m}, any={n_any_override} ({result["override_stats"]["pct_of_total"]:.1f}%)')

    # 被40MA降级交易的PnL统计 (这些交易原为trending, 被40MA降级为entangled)
    mask_override = _mask
    for hp in HOLD_PERIODS:
        pnl_col = f'pnl_pct_{hp}m'
        if pnl_col not in df_trades.columns:
            continue
        ov = df_trades[mask_override & df_trades[pnl_col].notna()]
        trn_valid = df_trades[(df_trades['combined_state'] == 'both_trending') & df_trades[pnl_col].notna()]
        if len(ov) >= 5:
            pnls = ov[pnl_col].values
            ov_mean = float(np.mean(pnls))
            result['override_stats'][f'override_trades_{hp}m'] = {
                'n': int(len(ov)),
                'mean_pnl_pct': round(ov_mean, 3),
                'win_rate': round(float((pnls > 0).mean() * 100), 1),
                'worst': round(float(np.min(pnls)), 3),
                'best': round(float(np.max(pnls)), 3),
            }
            if len(trn_valid) >= 5:
                trn_mean = float(np.mean(trn_valid[pnl_col].values))
                result['override_stats'][f'override_vs_trending_{hp}m'] = round(ov_mean - trn_mean, 3)
                print(f'  降级交易({hp}m): N={len(ov)}, 均PnL={ov_mean:.3f}%, WR={(pnls>0).mean()*100:.1f}% | vs真趋势: {ov_mean - trn_mean:+.3f}%')
...
def generate_report(all_results, output_path):
    ...
    total_overrides = sum(all_results[p].get('override_stats', {}).get('any_override', 0)
                          for p in products if p in all_results)
    total_trades = sum(all_results[p].get('total_trades', 0) for p in products if p in all_results)
    override_pct = round(total_overrides / total_trades * 100, 1) if total_trades > 0 else 0

    html = f"""<!DOCTYPE html>
<html><head>
<meta charset="utf-8">
<title>B044: 40MA趋势确认 — MA纠缠度增强验证 (期权PnL)</title>
...
<div class="header">
  <h1>B044: 40MA趋势确认 — 期权卖出PnL</h1>
  <p>卖出OTM宽跨(4-8%虚值), 持仓{hp}分钟, 按MA纠缠状态分组对比PnL</p>
  <p>基础: 1m-{MA_PERIOD_1M}MA + 5m-{MA_PERIOD_5M}MA, LB{LOOKBACK}/T{THRESHOLD} | 确认层: 1m-{MA_PERIOD_1M_CONFIRM}MA + 5m-{MA_PERIOD_5M_CONFIRM}MA</p>
  <p>规则: 20MA=trending + 40MA=entangled → 降级回纠缠</p>
...
<div class="card-row">
  ...
  <div class="card"><div class="val" style="color:#FF69B4">{total_overrides}</div><div class="label">40MA降级次数 ({override_pct}%)</div></div>
</div>
...
def main():
    parser = argparse.ArgumentParser(description='B044: 40MA趋势确认回测')
    ...
    print(f'B044: 40MA趋势确认回测')
    print(f'Products: {", ".join(p.upper() for p in products)}')
    print(f'Base: 1m-MA{MA_PERIOD_1M} + 5m-MA{MA_PERIOD_5M}, LB{LOOKBACK}/T{THRESHOLD}')
    print(f'Confirm: 1m-MA{MA_PERIOD_1M_CONFIRM} + 5m-MA{MA_PERIOD_5M_CONFIRM}')
    ...
    hp = HOLD_PERIODS[0]
    print(f'\n{"="*60}')
    print(f'B044 汇总 (持仓{hp}分钟, 40MA确认):')
    holds = 0
