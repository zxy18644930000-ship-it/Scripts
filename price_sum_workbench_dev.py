...
_LATEST_PRICE_SQL = """SELECT close_price FROM dbbardata WHERE symbol=?
    ORDER BY
        DATE(datetime) DESC,
        CASE WHEN CAST(substr(datetime,12,2) AS INTEGER) BETWEEN 9 AND 15 THEN 1 ELSE 0 END DESC,
        datetime DESC
    LIMIT 1"""
    ...
    if positions:
        ...
        if _pos_syms:
            try:
                ...
                _cur = _db.cursor()
                for _s in _pos_syms:
                    _cur.execute(_LATEST_PRICE_SQL, (_s,))
                    _row = _cur.fetchone()
...
def check_alert_resolved(call_sym, put_sym, sum_now, boll_middle):
    ...
    # 回归条件：价格之和 <= 布林中轨 * 1.05（接近或低于均值）
    threshold = boll_middle * 1.05
    if sum_now <= threshold:
        now_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        rec['resolved'] = True
...
def on_trade_entry_click(n_clicks, pair_json, volume, min_ask_sum, entry_direction, loading_state):
    """进仓按钮：POST /entry 到策略 HTTP API，支持卖出/买入方向"""
    print(f"[DEBUG 进仓] n_clicks={n_clicks}, entry_direction={entry_direction!r}, volume={volume}")
    if not n_clicks:
        return no_update
    if not pair_json:
    ...
    try:
        ...
        if ok:
            ...
            if min_ask_sum and min_ask_sum > 0:
                return html.Span(
                    f'条件挂单 Ask≥{min_ask_sum} {dir_label}各{volume}手 '
                    f'{pair["call"]}+{pair["put"]} ({ts})',
                    style={'color': '#FFD700'})
            return html.Span(
                f'已发送 {dir_label}各{volume}手 分{len(rounds)}轮{rounds} '
                f'{pair["call"]}+{pair["put"]} ({ts})',
                style={'color': '#00FF88'})
        else:
...
def on_trade_close_click(n_clicks, pair_json, volume, max_bid_sum, close_direction):
    ...
    try:
        ...
        if ok:
            ...
            if max_bid_sum and max_bid_sum > 0:
                return html.Span(
                    f'条件平仓 Bid≤{max_bid_sum} {dir_label}平仓各{volume}手 '
                    f'{pair["call"]}+{pair["put"]} ({ts})',
                    style={'color': '#FFD700'})
            return html.Span(
                f'已发送 {dir_label}平仓各{volume}手 分{len(rounds)}轮{rounds} '
...
def toggle_plan(n_clicks):
    ...
    for m in matched:
        ...
        prod_best_sharpe = max((info[bn]['sharpe'] for bn, _, _ in _DTE_RANGES
                                if bn in _V6_CONFIG.get(prod, {})),
                               default=base_sharpe)
        ...
        # 仓位分配
        alloc = min(account * 0.03, account / max(len(matched), 1))
        ...
        m.update({
            ...
            'total_margin': total_margin, 'pct': total_margin / account,
            'composite': composite,
    ...
    header = html.Tr([
        ...
        html.Th('品种', style={'width': '75px'}),
        html.Th('合约', style={'width': '65px'}),
        html.Th('DTE', style={'width': '40px', 'textAlign': 'center'}),
        html.Th('综合分', style={'width': '60px', 'textAlign': 'center'}),
        html.Th('Sharpe', style={'width': '55px', 'textAlign': 'center'}),
        html.Th('VRP', style={'width': '45px', 'textAlign': 'center'}),
        html.Th('新闻', style={'width': '55px', 'textAlign': 'center'}),
        html.Th('波动', style={'width': '45px', 'textAlign': 'center'}),
        html.Th('流动', style={'width': '40px', 'textAlign': 'center'}),
        html.Th('胜率', style={'width': '45px', 'textAlign': 'center'}),
        html.Th('入场', style={'width': '55px', 'textAlign': 'center'}),
        html.Th('TP/SL', style={'width': '75px', 'textAlign': 'center'}),
        html.Th('手/保证金', style={'width': '90px', 'textAlign': 'right'}),
    ], style={'backgroundColor': '#1a1a3e', 'color': '#4fc3f7', 'fontSize': '11px'})
    ...
    for rank, p in enumerate(picks):
        ...
        row_bg = '#1a2a1a' if is_best else '#0d1117'
        mark = f'{rank+1}' if is_best else ''
        dte = p['dte']
        dte_color = '#f44336' if dte <= 5 else '#ff9800' if dte <= 10 else '#4caf50' if dte <= 30 else '#2196f3'

        cs = p['composite']
        cs_color = '#00e676' if cs >= 30 else '#4fc3f7' if cs >= 15 else '#ff9800' if cs >= 8 else '#f44336'

        sh = p['sharpe']
        sh_color = '#00e676' if sh >= 20 else '#4fc3f7' if sh >= 10 else '#ff9800'

        # VRP颜色
        vf = p['vrp_factor']
        vrp_c = '#00e676' if vf >= 1.3 else '#4fc3f7' if vf >= 1.1 else '#888'

        # 新闻颜色
        nf = p['news_factor']
        news_c = '#f44336' if nf <= 0.5 else '#ff9800' if nf <= 0.7 else '#4caf50'

        # 波动颜色
        vol_c = '#4caf50' if p['vol_factor'] >= 1.2 else '#888' if p['vol_factor'] >= 1.0 else '#f44336'

        # 流动性颜色
        liq_c = '#4caf50' if p['liq_factor'] >= 1.0 else '#ff9800' if p['liq_factor'] >= 0.85 else '#f44336'

        wr = p['wr']
        wr_color = '#00e676' if wr >= 90 else '#4fc3f7' if wr >= 80 else '#ff9800'

        entry_label = _ENTRY_LABELS.get(p['entry'], p['entry'])
        sl_text = '无' if p['sl'] >= 999 else f'{p["sl"]:.1f}x'
        tp_sl = f'{p["tp"]:.1f}θ/{sl_text}'
        wday_mark = p['wday_label']

        total_mgn += p['total_margin']
        total_lots += p['lots']

        name_text = f"{p['name']}"
        if wday_mark:
            name_text += f' {wday_mark}'

        rows.append(html.Tr([
            html.Td(mark, style={'color': '#ff9800', 'fontWeight': 'bold', 'fontSize': '13px', 'textAlign': 'center'}),
            html.Td(name_text, style={'color': '#fff' if is_best else '#aaa', 'fontWeight': 'bold' if is_best else 'normal', 'fontSize': '12px'}),
            html.Td(p['contract'], style={'color': '#888', 'fontSize': '11px'}),
            html.Td(str(dte), style={'textAlign': 'center', 'color': dte_color, 'fontWeight': 'bold'}),
            html.Td(f"{cs:.1f}", style={'textAlign': 'center', 'color': cs_color, 'fontWeight': 'bold', 'fontSize': '13px'}),
            html.Td(f"{sh:.0f}", style={'textAlign': 'center', 'color': sh_color}),
            html.Td(p['vrp_label'], style={'textAlign': 'center', 'color': vrp_c, 'fontSize': '11px'}),
            html.Td(p['news_label'], style={'textAlign': 'center', 'color': news_c, 'fontSize': '10px'}),
            html.Td(p['vol_label'], style={'textAlign': 'center', 'color': vol_c, 'fontSize': '11px'}),
            html.Td(p['liq_label'], style={'textAlign': 'center', 'color': liq_c, 'fontSize': '11px'}),
            html.Td(f"{wr:.0f}%", style={'textAlign': 'center', 'color': wr_color, 'fontSize': '11px'}),
            html.Td(entry_label, style={'textAlign': 'center', 'color': '#888', 'fontSize': '10px'}),
            html.Td(tp_sl, style={'textAlign': 'center', 'color': '#888', 'fontSize': '10px'}),
            html.Td(f"{p['lots']}手/{p['total_margin']/10000:.1f}万", style={'textAlign': 'right', 'color': '#4fc3f7', 'fontSize': '11px'}),
        ], style={'backgroundColor': row_bg, 'borderBottom': '1px solid #1a1a3e'}))

    total_pct = total_mgn / account if account > 0 else 0

    summary_row = html.Tr([
        html.Td(''),
        html.Td(f'{len(seen_prods)}品种{len(picks)}合约', style={'color': '#ff9800', 'fontWeight': 'bold', 'fontSize': '11px'}),
        html.Td(''), html.Td(''),
        html.Td('综合', style={'color': '#ff9800', 'textAlign': 'center', 'fontSize': '11px'}),
        html.Td(''), html.Td(''), html.Td(''), html.Td(''), html.Td(''), html.Td(''), html.Td(''), html.Td(''),
        html.Td(f'{total_lots}手/{total_mgn/10000:.0f}万({total_pct:.0%})',
    ...
    if picks:
        plan_sections.extend([
            html.Div([
                ...
                html.Span(f'  {today_str} {now_time}  8维度综合评分  账户{account/10000:.0f}万',
                           style={'color': '#666', 'fontSize': '12px'}),
            ], style={'padding': '10px 25px', 'borderBottom': '1px solid #2a2a4a'}),
            html.Div(table, style={'padding': '5px 15px', 'overflowX': 'auto'}),
            html.Div([
                html.Span('综合分 = Sharpe × VRP × 新闻 × 近期波动 × 流动性 × 周度 × DTE甜蜜区 × 胜率 | ',
                           style={'color': '#555', 'fontSize': '10px'}),
                html.Span(' | '.join(factor_desc), style={'color': '#ff9800', 'fontSize': '10px'}),
            ], style={'padding': '6px 25px'}),
        ])
    else:
        plan_sections.append(html.Div('当前无v6匹配的交易机会',
            style={'color': '#888', 'padding': '30px', 'textAlign': 'center'}))
    ...
    for prod, cn, ex, contract, dte, bucket, sharpe, wr, entry, tp, sl, n in matched:
        ...
        for c_sym, c_px, c_vol, c_k in otm_calls[:10]:
            for p_sym, p_px, p_vol, p_k in otm_puts[:10]:
                ...
                # === 评分：虚值度对称 × OTM范围 × 瓶颈腿成交额 ===
                # 1. 虚值度对称性：两腿OTM%越接近越好（宽跨允许一定不对称）
                avg_otm = (c_otm_pct + p_otm_pct) / 2
                otm_diff_ratio = abs(c_otm_pct - p_otm_pct) / avg_otm
                sym_score = max(0.3, 1.0 - otm_diff_ratio * 0.3)
                # 2. 目标虚值度偏好：4%-8%是宽跨甜点区
                otm_penalty = ((avg_otm - 0.06) / 0.05) ** 2
                otm_range_score = max(0.1, 1.0 - otm_penalty * 0.3)
                # 3. 瓶颈腿成交额（价格×成交量，衡量真实可执行性）
                #    成交额<几万的期权bid-ask宽、挂单薄，根本无法有效执行
                import math as _math
                _c_to = c_px * c_vol
                _p_to = p_px * p_vol
                _min_to = min(_c_to, _p_to)
                vol_score = max(0, min((_math.log10(max(_min_to, 10)) - 4.0) / 1.1, 1.0))
                # 4. 单腿最低权利金（防止手续费占比过大）
                psum_score = 1.0 if min(c_px, p_px) >= 2 else 0.3
                composite = sharpe * sym_score * otm_range_score * (0.15 + 0.85 * vol_score) * psum_score
    ...
    # === 构建精选表格 ===
    v6_hdr = html.Tr([
        html.Th('#', style={'width': '25px', 'padding': '6px 4px'}),
        html.Th('品种', style={'width': '90px', 'padding': '6px 8px'}),
        html.Th('期权对', style={'width': '180px', 'padding': '6px 4px'}),
        html.Th('C价', style={'width': '50px', 'textAlign': 'right', 'padding': '6px 4px'}),
        html.Th('P价', style={'width': '50px', 'textAlign': 'right', 'padding': '6px 4px'}),
        html.Th('权金和', style={'width': '55px', 'textAlign': 'right', 'padding': '6px 4px'}),
        html.Th('腿比', style={'width': '40px', 'textAlign': 'center', 'padding': '6px 4px'}),
        html.Th('成交量', style={'width': '50px', 'textAlign': 'center', 'padding': '6px 4px'}),
        html.Th('DTE', style={'width': '35px', 'textAlign': 'center', 'padding': '6px 4px'}),
        html.Th('入场', style={'width': '55px', 'padding': '6px 4px'}),
        html.Th('TP/SL', style={'width': '70px', 'padding': '6px 4px'}),
        html.Th('Sharpe', style={'width': '50px', 'textAlign': 'center', 'padding': '6px 4px'}),
        html.Th('胜率', style={'width': '42px', 'textAlign': 'center', 'padding': '6px 4px'}),
    ], style={'backgroundColor': '#1a1a3e', 'color': '#4fc3f7', 'fontSize': '11px'})

    v6_rows = []
    for i, p in enumerate(top_picks):
        dte_c = '#f44336' if p['dte'] <= 7 else '#ff9800' if p['dte'] <= 14 else '#4caf50'
        sh_c = '#00e676' if p['sharpe'] >= 20 else '#4fc3f7' if p['sharpe'] >= 12 else '#ff9800'
        wr_c = '#00e676' if p['wr'] >= 90 else '#4fc3f7' if p['wr'] >= 80 else '#ff9800'
        ratio_c = '#00e676' if p['ratio'] <= 1.3 else '#4fc3f7' if p['ratio'] <= 1.8 else '#ff9800'
        vol_c = '#00e676' if p['total_vol'] >= 100 else '#ff9800' if p['total_vol'] >= 20 else '#f44336'
        tp_l = f'{p["tp"]:.1f}θ' if p['tp'] < 900 else '无'
        sl_l = f'{p["sl"]:.0f}x' if p['sl'] < 900 else '无'
        bg = '#0d1a0d' if i < 5 else '#0d1117'
        rank_c = '#FFD700' if i < 3 else '#00FF88' if i < 5 else '#aaa'
        v6_rows.append(html.Tr([
            html.Td(f'{i+1}', style={'color': rank_c, 'fontWeight': 'bold', 'textAlign': 'center', 'padding': '4px'}),
            html.Td(f'{p["cn"]}', style={'color': '#fff', 'fontWeight': 'bold', 'padding': '4px 8px'}),
            html.Td([
                html.Span(f'C{p["call_k"]}', style={'color': '#00BFFF'}),
                html.Span(f'+', style={'color': '#555'}),
                html.Span(f'P{p["put_k"]}', style={'color': '#FF6B6B'}),
                html.Span(f' ({(p["call_k"]-p["fut_px"])/p["fut_px"]*100:.0f}%/{(p["fut_px"]-p["put_k"])/p["fut_px"]*100:.0f}%)',
                           style={'color': '#666', 'fontSize': '10px'}),
            ], style={'fontSize': '11px', 'padding': '4px'}),
            html.Td(f'{p["call_px"]:.0f}', style={'textAlign': 'right', 'color': '#00BFFF', 'padding': '4px'}),
            html.Td(f'{p["put_px"]:.0f}', style={'textAlign': 'right', 'color': '#FF6B6B', 'padding': '4px'}),
            html.Td(f'{p["psum"]:.0f}', style={'textAlign': 'right', 'color': '#FFD700', 'fontWeight': 'bold', 'padding': '4px'}),
            html.Td(f'{p["ratio"]:.1f}', style={'textAlign': 'center', 'color': ratio_c, 'padding': '4px'}),
            html.Td(f'{p["total_vol"]:.0f}', style={'textAlign': 'center', 'color': vol_c, 'padding': '4px'}),
            html.Td(str(p['dte']), style={'textAlign': 'center', 'color': dte_c, 'fontWeight': 'bold', 'padding': '4px'}),
            html.Td(_entry_labels.get(p['entry'], p['entry']), style={'color': '#aaa', 'fontSize': '11px', 'padding': '4px'}),
            html.Td(f'{tp_l}/{sl_l}', style={'color': '#888', 'fontSize': '11px', 'padding': '4px'}),
            html.Td(f'{p["sharpe"]:.0f}', style={'textAlign': 'center', 'color': sh_c, 'fontWeight': 'bold', 'padding': '4px'}),
            html.Td(f'{p["wr"]:.0f}%', style={'textAlign': 'center', 'color': wr_c, 'padding': '4px'}),
        ], style={'backgroundColor': bg, 'borderBottom': '1px solid #1a1a3e'}))

    # 被过滤掉的数量
    n_filtered = len(matched) - len(actionable)
    ...
    plan_sections.append(html.Div([
        html.Div([
            html.Span(f'精选OTM宽跨 — Top {len(top_picks)}', style={
                'color': '#FFD700', 'fontSize': '15px', 'fontWeight': 'bold'}),
            html.Span(f'  从{len(matched)}个信号中筛出{len(actionable)}个可交易({n_filtered}个无行情/无OTM对)',
                       style={'color': '#666', 'fontSize': '11px'}),
        ], style={'padding': '10px 25px', 'borderBottom': '1px solid #4a4a0a'}),
        html.Div(v6_table, style={'padding': '5px 20px', 'overflowX': 'auto'}),
        html.Div([
            html.Span('已按 Sharpe×流动性×腿比×虚值对称 综合排序 | OTM 2%~15% | 腿比≤2.5 | Top5推送至智能推荐',
                       style={'color': '#666', 'fontSize': '11px'}),
            html.Span(' | N入→次日15:00前平 D入→当日15:00前平',
                       style={'color': '#888', 'fontSize': '11px'}),
        ], style={'padding': '8px 25px'}),
    ], style={'borderTop': '2px solid #FFD700', 'backgroundColor': '#1a1a0a'}))
